var loadAWSC = function() {
  console.log("AWSC is typeof " + typeof AWSC);
  if (confirmLoadedAWSCRetry >= 0) {
    confirmLoadedAWSCRetry--;
    if (typeof AWSC === "undefined") {
      console.log("AWSC doesn't exist yet... waiting patiently");
      setTimeout(loadAWSC, 1000);
    } else {
      var navType = typeof AWSC.jQuery("#nav-usernameMenu .nav-elt-label")[0]
        .style;
      if (navType === "undefined") {
        console.log("Nav doesn't exist yet... waiting patiently");
        setTimeout(loadAWSC, 1000);
      }

      AWSC.jQuery("#nav-usernameMenu .nav-elt-label")[0].style.color = "#fff";
      AWSC.jQuery("#nav-usernameMenu .nav-elt-label")[0].style.textShadow =
        "#232f3e 0 0 2px";
      ...
      (function(g, e, l) {
        ...
        function b(B) {
          var A = decodeURIComponent(e.trim(B))
            .replace(/^federated-user\//, "")
            .replace(/^assumed-role\//, "")
            .replace(/&#\d{1,5};/g, function(C) {
        ...
        var I = f(ConsoleNavService.Util.getCookie("aws-userInfo")),
          ...
          N = i(F),
          M = {
            roleDisplayNameAccount: I.alias,
            roleDisplayNameUser: I.arn.split("/").slice(-2, -1)[0]
          },
        ...
        for (role in document.extraRoles) {
          var accid = document.extraRoles[role]
            .split("/")[0]
            .split(":")
            .slice(-2, -1)[0];
        ...
        AWSC.jQuery(AWSC).trigger({
          ...
          signinEndpoint: k(
            AWSC.jQuery("meta#awsc-signin-endpoint").attr("content")
          ),
      ...
      AWSC.jQuery("#nav-usernameMenu").on("click", function(event) {
        ...
        if (
          AWSC.jQuery("#nav-usernameMenu")[0].classList.contains("active") ==
          false
        ) {
    ...
    console.log("AWSC hasn't loaded still, giving up and continuing anyway.");
  }
};

var confirmLoadedAWSCRetry = 10;
